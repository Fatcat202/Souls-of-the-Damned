...
#region Attack Stats
	...
	function a_stats(_dmg_die_num = 0, _dmg_die_sides = 0, _dmg_mod = 0, _dmg_die_num_per_lvl = 0, _dmg_die_sides_per_lvl = 0, _dmg_mod_per_lvl = 0, _cooldown = 0, _duration = 0, _kb_length = 0, _crit_chance = 0, _crit_mod = 0, _scr = -1, _spr = spr_placeholder, _title = "Empty", _desc = "Empty") constructor {
		...
		dmg_mod = _dmg_mod;
		dmg_die_num_per_lvl = _dmg_die_num_per_lvl;
		dmg_die_sides_per_lvl = _dmg_die_sides_per_lvl;
	...
	for(var i = 0; i < global.attack_index_length; i++)
	{
		...
		global.attack_stats[yy].dmg_die_num_per_lvl = real(ds_grid_get(ds_attack_stats_csv, xx, yy)); xx++;
		global.attack_stats[yy].dmg_die_sides_per_lvl = real(ds_grid_get(ds_attack_stats_csv, xx, yy)); xx++;
	...
	// Set base dmg die variables for leveling
	for(var i = 1; i < global.attack_index_length + 1; i++)
	{
		global.attack_stats[i].dmg_die_num_base = global.attack_stats[i].dmg_die_num;
		global.attack_stats[i].dmg_die_sides_base = global.attack_stats[i].dmg_die_sides;
		global.attack_stats[i].dmg_mod_base = global.attack_stats[i].dmg_mod;
	}
